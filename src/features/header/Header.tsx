...
export const ProfileWrapper: FC<ProfileWrapper> = (props): JSX.Element => {
  ...
  const surnameFirstChar =
    accounts?.length && accounts[0]?.name ? accounts[0]?.name.split(' ')[1].split('')[0] : '';
  return (
    <div
      className='profile'
      style={{
    ...
    >
      ...
      <div className='profileWrapper'>
        ...
        <div>
          {firstName || surnameFirstChar ? (
            <p className='fullName'>{getName(firstName, surnameFirstChar, false)}</p>
          ) : (
            ''
          )}
